...
template<typename Dtype>
void PoseDataLayer<Dtype>::prepareRPNtargets(std::string filename,
        ...
        float scale)
{
    ...
    RectF target_rect(cv::Point2f(min_x-margin, min_y-margin),
                      cv::Point2f(max_x+margin, max_y+margin));
...
//                 << w_s << " " << h_s;
    ...
    for (int j = 0; j < truncated_height; ++j) {
      for (int i = 0; i < truncated_width; ++i) {
        ...
        cv::Point2f diff = cv::Point2f(x_s, y_s) - pt;
