...
void SmoothL1LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  sample_count_ = 0;
  Dtype count_samples = 0;
  caffe_gpu_asum(count, bottom[2]->gpu_data(), &count_samples);
  sample_count_ = count_samples;

  Dtype loss;
  caffe_gpu_asum(count, errors_.gpu_data(), &loss);
  if(sample_count_ != 0)
    top[0]->mutable_cpu_data()[0] = loss / sample_count_;
}
...
void SmoothL1LossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < 2; ++i) {
    if (propagate_down[i]) {
      ...
      const Dtype alpha = sign * top[0]->cpu_diff()[0] / std::max(sample_count_, 100);
      caffe_gpu_axpby(
